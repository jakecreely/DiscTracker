...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--end-date", type=str, default=str(date.today().strftime("%Y-%m-%d"))
        )
    ...
    def handle(self, *args, **options):
        ...
        if count > self.MAX_COUNT:
            raise CommandError(
                f"Error: --count cannot be greater than {self.MAX_COUNT}"
            )

        if min_price < self.MIN_PRICE:
            raise CommandError(
                f"Error: --min-price cannot be less than {self.MIN_PRICE}"
            )

        if max_price > self.MAX_PRICE:
            raise CommandError(
                f"Error: --max-price cannot be greater than {self.MIN_PRICE}"
            )
        ...
        try:
            ...
            raise CommandError(
                "Error: Dates must be in format YYYY-MM-DD (e.g., 2024-02-10)"
            )

        if start_date > end_date:
            raise CommandError("Error: --start-date cannot be after --end-date")

        self.seed_random_price_history_entries(
            count, start_date, end_date, min_price, max_price
        )
        print("Successfully seeded random price history entries")

    def seed_random_price_history_entries(
        self, count, start_date, end_date, min_price, max_price
    ):
